...
// GET /api/documents — list documents for a matter (paginated, filtered)
router.get('/', async (req, res, next) => {
  try {
    ...
    const validSort = ['created_at','original_name','file_size','date_modified','email_date','processing_stage'].includes(sortBy)
      ? sortBy : 'created_at';
    ...
    const [rowsResult, countResult] = await Promise.all([
      db.query(
        `SELECT
           ...
           d.md5, d.sha256,
           d.is_duplicate, d.is_near_dup, d.is_nist, d.is_privileged,
           d.is_responsive, d.review_status,
           d.has_ocr, d.ocr_confidence,
           d.email_from, d.email_to, d.email_subject, d.email_date,
           d.author, d.date_created, d.date_modified,
           d.custodian_name, d.language,
           d.page_count, d.word_count, d.text_length,
...
router.get('/:id', async (req, res, next) => {
  try {
    const result = await db.query(`
      SELECT d.*,
        ARRAY(
          SELECT json_build_object('id', t.id, 'name', t.name, 'label', t.label, 'color', t.color,
                                   'group_name', t.group_name, 'applied_at', dt.applied_at, 'applied_by', dt.applied_by)
...
router.get('/:id/chain-of-custody', async (req, res, next) => {
  try {
    const result = await db.query(`
      SELECT id, entry_hash, prev_hash, event_type, user_name, user_role,
             ip_address, description, metadata, occurred_at
...
// GET /api/documents/:id/family — parent + siblings (attachments)
router.get('/:id/family', async (req, res, next) => {
  try {
    ...
    const result = await db.query(`
      ...
      FROM documents
      WHERE matter_id = $1 AND (id = $2 OR parent_id = $2)
...
// POST /api/documents/:id/tags — apply a tag
router.post('/:id/tags', async (req, res, next) => {
  try {
    ...
    // If exclusive, remove other tags in same group
    ...
    await db.query(
      `INSERT INTO document_tags (document_id, tag_id, matter_id, applied_by, notes)
       VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT (document_id, tag_id) DO UPDATE SET notes = EXCLUDED.notes`,
      [req.params.id, tagId, doc.rows[0].matter_id, req.user?.id, notes]
    );

    await logDocumentTagged({
      matterId:   doc.rows[0].matter_id,
      documentId: req.params.id,
      userId:     req.user?.id,
      userName:   req.user?.name,
...
// DELETE /api/documents/:id/tags/:tagId — remove a tag
router.delete('/:id/tags/:tagId', async (req, res, next) => {
  try {
    ...
    await logEvent({
      ...
      userId:      req.user?.id,
      userName:    req.user?.name,
      description: `Tag "${tag.rows[0]?.name}" removed from document`,
    });
...
// PATCH /api/documents/:id — update review fields
router.patch('/:id', async (req, res, next) => {
  try {
    const {
      reviewStatus, isResponsive, isPrivileged, privilegeType, privilegeBasis,
      isWithheld, notes,
    } = req.body;
